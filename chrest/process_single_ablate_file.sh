...
fields_list="monitor_energySource:souener monitor_Yi:Yi aux_pressure:pressure aux_temperature:temp solution_euler:euler"
if [[ $chemtab_output ]]; then
    echo answer: True
    fields_list="$fields_list aux_Progress:mass_ monitor_progressSource:source_" # solution_densityProgress:density_"
    # results in: mass_CPV_PC* & source_CPV_PC*, verified 10/4/23
else
    echo answer: False
    fields_list="$fields_list monitor_yiSource:souspec monitor_zMix:zmix" # solution_densityYi:density_"
fi
