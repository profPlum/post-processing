...
if (!silent) cat('input_fn: ', input_fn, '\\n')
...
if (!silent) cat('(before) mean(temp_by_group$is_flame): ', mean(temp_by_group$is_flame), '\n')
temp_by_group = temp_by_group %>% group_by(is_flame) %>% slice_sample(n=n_flame)
if (!silent) cat('(after) mean(temp_by_group$is_flame): ', mean(temp_by_group$is_flame), '\n')
...
if (!silent) print(c('colnames: ', colnames(df)))

# write back to input file
out_fn=sub('\\.csv.*$', '-rebalanced.csv.gz', input_fn)
if (!silent) cat('output file:', out_fn)
write.csv(df, out_fn)
